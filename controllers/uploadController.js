...
export const deleteFile = async (req, res) => {
  ...
  try {
    ...
    const deletedFile = await LetterHead.findOneAndDelete({ image_id: public_id });
...
try {
  const files = await LetterHead.find().select("file_link image_id subject body_text -_id");
...
try {
  ...
  const oldFile = await LetterHead.findOne({ image_id: public_id });
  ...
  oldFile.image_id = result.public_id;
